...
Going back to the original later works, but the quests it finds will be ones it doesn't recognize, so they sit unchanged until the next day rolls them over.

## Installing

There are two ways to install this add-on:

1. Via [AnkiWeb](https://ankiweb.net/shared/info/305971550) as every other add-on. This way the version will automatically stay up to date.
2. Download the `.ankiaddon` file from [releases](https://github.com/saraphl/collectquest/releases) and either open it as an Anki program directly, or in Anki window navigate to **Tools → Add-ons → Install from file...**.
