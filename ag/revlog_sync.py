...
def _fetch_revlog_rows(col, last_id: int) -> list[tuple[int, int]]:
    """
    Fetch revlog rows with id > last_id. Uses ONLY db.execute() (not db.list()).
    Returns list of (id, ease). Logs to revlog_debug.log.
    """
    ...
    chunk_size = 1000
    while True:
        sql = f"SELECT id, ease FROM revlog WHERE id > {last_id} ORDER BY id LIMIT {chunk_size}"
        _log(f"  sql: {sql}")
        try:
            res = db.execute(sql)
        ...
        if not rows:
            _log("  no rows, done")
            break
        # Normalize: flat [id,e,id,e,...] or [(id,e),...]
        first = rows[0]
        ...
        for rid, ease in pairs:
            ...
            if 0 <= e <= 4:
                out.append((rid, e))
        if pairs:
            last_id = max(rid for rid, _ in pairs)
...
def _ease_from_revlog(ease: int) -> int:
    """Revlog ease: 0 or 1 = Again, 2 = Hard, 3 = Good, 4 = Easy. Normalize 0 -> 1 for our XP logic."""
    if ease == 0:
        return 1
...
def _process_synced_revlog_impl(col, silent: bool) -> dict | None:
    ...
    total_gems = 0
    applied = 0
    for revlog_id, revlog_ease in rows:
        ...
        # Append to undo buffer so multiple Ctrl+Z can revert synced reviews too
        if ease > 1:
            try:
                from aqt import mw
